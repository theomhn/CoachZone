...
#[AsController]
class AddFavoriteInstitutionController extends AbstractController
{
    ...
    public function __invoke(string $institutionId, #[CurrentUser] Coach $coach): JsonResponse
    {
        ...
        if ($coach->isFavoriteInstitution($institution)) {
            return new JsonResponse([
                'success' => false,
                'message' => 'Institution déjà en favoris',
                'institutionId' => $institutionId,
                'isFavorite' => true
            ], 409); // Conflict
        }

        $coach->addFavoriteInstitution($institution);
        $this->entityManager->flush();

        return new JsonResponse([
            'success' => true,
            'message' => 'Institution ajoutée aux favoris',
            'institutionId' => $institutionId,
            'isFavorite' => true
        ], 201); // Created
    }
